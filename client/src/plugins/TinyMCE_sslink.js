...
const plugin = {
  ...
  init(editor) {
    ...
    const title = i18n._t('TinyMCE.INSERT_LINK', 'Insert link');
    const titleWithShortcut = i18n.inject(
      i18n._t('TinyMCE.INSERT_LINK_WITH_SHORTCUT', 'Insert link {shortcut}'),
      { shortcut: `[${metaKey}+K]` }
    );
    ...
    editor.ui.registry.addButton('sslink-edit', {
      text: i18n._t('TinyMCE.EDIT_LINK', 'Edit link'),
      onAction: openLinkDialog,
    });
    editor.ui.registry.addButton('sslink-remove', {
      text: i18n._t('TinyMCE.REMOVE_LINK', 'Remove link'),
...
jQuery.entwine('ss', ($) => {
  $('.insert-link__dialog-wrapper').entwine({
    ...
    checkNodeMatches(node, matchWith) {
      ...
      // In some browsers, selecting an image actually results in selecting the paragraph.
      // Treat the selected element as a match when it is a direct child of the reported node.
